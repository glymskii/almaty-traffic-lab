...
`right` −30…−150°, иначе `uturn`; выезд обратно на то же плечо всегда `uturn`. Из нескольких выездов одного вида
берётся ближайший к идеальному углу (0° / ±90° / 180°), для `merge` — самый прямой.
...
(`turns: [merge]`, `endS < lengthM`) — цель только для `merge`. Геометрия: кубическая Безье, контрольные точки на
