...
## Цель
`pnpm run import --bbox small|big` скачивает данные OSM для полигона через Overpass плитками, с повторами и зеркалами,
...
7. Выполнить `pnpm run import --bbox small` и закоммитить снимок. Выполнить `pnpm run import --bbox big`; если суммарный размер
...
- [ ] `pnpm run import --bbox small` создаёт `snapshot.json.gz`, повторный запуск использует кэш и даёт байт-в-байт тот же снимок.
