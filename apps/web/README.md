...
рекомендациями, спарклайн задержки, мини-карта). Сам ничего не считает про движение — только читает
буферы кадров и метрики (`sim-core`/`sim-worker`).
...
Содержимое вкладки «Сравнение» — заглушка со ссылкой на T-26 (навигация между вкладками уже работает).
Панель А/Б сравнения двух сценариев в одном 3D-окне (D12) — T-26.
